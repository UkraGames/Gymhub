...
public class DaoClientImpl extends GymConnect implements DaoClient {
    ...
    public void modifyClient(Client e) throws Exception {
        try {
            this.getConnection();
            String sql = "UPDATE client SET nameClient= ?, startDate = ?, finalDate = ?, subState = ?, subType = ?, number = ?, eMail = ?, height = ?, weight = ?, payment WHERE documentClient = ?";
            PreparedStatement up = conn.prepareStatement(sql);
            up.setString(1, e.getNameClient());
            up.setString(2, e.getStartDate());
            up.setString(3, e.getFinalDate());
            up.setInt(4, e.getSubState());
            up.setInt(5, e.getSubType());
            up.setString(6, e.getNumber());
            up.setString(7, e.geteMail());
            up.setFloat(8, e.getHeight());
            up.setFloat(9, e.getWeight());
            up.setInt(10, e.getPayment());
            up.setInt(12, e.getDocumentClient());
            up.executeUpdate();
        }catch(SQLException ex){
            ex.printStackTrace();
        } finally {
